...
import recommendCompanies from './recommendCompanies';
...
export default function RecommendationsPage() {
  ...
  useEffect(() => {
    // コンポーネントマウント時に即時実行関数でasync/awaitを使用
    (async () => {
      try {
        const res = await recommendCompanies();
        console.log(JSON.parse(res));
        const res_data = JSON.parse(res)
        recommendedCompanies[0].title = res_data[0].entity_title;
        recommendedCompanies[0].company = res_data[0].company_name;
        recommendedCompanies[0].representative.name = res_data[0].user_name;
        recommendedCompanies[0].tags = res_data[0].tags.split("、");
        recommendedCompanies[1].title = res_data[1].entity_title;
        recommendedCompanies[1].company = res_data[1].company_name;
        recommendedCompanies[1].representative.name = res_data[1].user_name;
        recommendedCompanies[1].tags = res_data[1].tags.split("、");
        setLoading(false);
      } catch (error) {
  ...
  // ここはコンポーネントのreturn、描画のためのJSXを返す場所
  if (loading) {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div
            ...
            src="https://udify.app/chatbot/Gc6jOcG7InBF41Ec"
            frameborder="0"
            allow="microphone">
            </iframe> 
